...
<html>
<head>
    <title>毕设选题系统 - 管理员发布通知管理页面</title>
    <meta charset="UTF-8">
...
<div class="x_panel" >
    ...
    <div class="x_content" >
        <div id="toolbar">
            <a href="${ctx}/sys/admin/deliverNoticeAdd" class="btn btn-success btn-sm"><i class="fa fa-paper-plane"></i> 发布通知</a>
        </div>
...
<script>
    var list_url = '${ctx}/sys/admin/getNoticeList';
    // 初始化表格数据
    ...
    function noticeDelete(s) {
        window.parent.layer.confirm("确认删除?", {icon: 3, offset: 't'}, function () {
            $.ajax({
                url: '${ctx}/sys/admin/noticeDelete/' + s,
                type: 'get',
                success: function (response) {
                    if (response.code == 0) {
                        window.parent.layer.msg(response.msg, {icon: 1, time: 1000, offset: 't'});
                        window.location.href = '${ctx}/sys/admin/notice';
                    } else {
    ...
    function getNoticeOneData(s) {
        $.ajax({
            url: '${ctx}/sys/admin/getNoticeOneData/'+s,
            type: 'get',
            success: function (response) {
                if (response.code == 0) {
                    $("#pathName").html("")
                    ...
                    for(var i=0;i<response.notice.pathName.length;i++){
                      ...
                      $("#pathName").append('<span><b>'+path+'</b>&nbsp;&nbsp;<a href=\"${ctx}/sys/admin/downloadFile/'+response.notice.pathName[i]+'\">下载</a></span><br/>');
                    }
                    $("#myModal").modal();
                } else {
                    window.parent.layer.alert(response.msg, {icon: 5, offset: 't'});
