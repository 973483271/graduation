...
<div class="x_panel">
    ...
    <div class="x_content">
        <div id="toolbar" class="btn-toolbar">
            <div class="btn-group">
                <a href="${ctx}/sys/admin/addTeacher" class="btn btn-default btn-sm"><i class="fa fa-plus"></i>新增</a>
...
<script>
    var list_url = '${ctx}/sys/admin/teachersList';
    // 初始化表格数据
    ...
    $('#btn-update').click(function () {
        ...
        if (rows.length == 0) {
            ...
            window.location.href = '${ctx}/sys/admin/teachersUpdate/' + rows[0].teaId;
        }
    });
    ...
    $('#btn-delete').click(function () {
        ...
        if (rows.length == 0) {
            ...
            window.parent.layer.confirm("确认删除?", {icon: 3, offset: 't'}, function () {
                $.ajax({
                    url: '${ctx}/sys/admin/teachersDelete/' + rows[0].teaId,
                    type: 'get',
            ...
            window.parent.layer.confirm("确认批量删除?", {icon: 3, offset: 't'}, function () {
                ...
                for (var i = 0; i < rows.length; i++) {
                    ids.push(rows[i].teaId);
                }
                $.ajax({
                    url: '${ctx}/sys/admin/teachersDeleteBatch',
    ...
    $("#btn-batchAdd").click(function () {
        alert("请选择字段名为(用户名,姓名,学院(部),职称,联系电话,邮箱)的Excel表导入");
        $("#importExel").click();
    });
    ...
    $(document).off('change','#importExel').on('change','#importExel',function(){
        window.parent.layer.confirm("确认导入?", {icon: 3, offset: 't'}, function () {
            ...
            $.ajax({
                url: '${ctx}/sys/admin/teacherBatchAdd',
                type: 'post',
