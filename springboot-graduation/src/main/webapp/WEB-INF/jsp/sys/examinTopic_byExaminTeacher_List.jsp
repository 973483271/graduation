...
<html>
<head>
    <title>毕设选题系统 - 审核组老师审核导师上交的题目</title>
    <meta charset="UTF-8">
...
<div class="page-title">
    ...
    <div class="title_right" style="z-index: 1000;" >
        <span style="font-size: 15px;position: absolute;right:110px;color: #FFFFFF">审题剩余:</span><br>
...
<script>
    var list_url = '${ctx}/sys/examinteacher/examinTopicList';
    // 初始化表格数据
    var dataTable = $('#data-table').bootstrapTable({
        ...
        columns: [{
            ...
            }},
            {
                ...
                title: '操作',
                formatter: function(value){
                    return '<shiro:hasPermission name="sys:admin:check"><a href="javascript:void(0);"  class="btn btn-info btn-sm" style="margin-bottom: 0px"  onclick="examinTopicPass(\''+value+'\')">'+ '<span><i class="fa fa-check"/>通过</span></a>'
                        +'<a href="javascript:void(0);"  class="btn btn-danger btn-sm" style="margin-bottom: 0px"  onclick="examinTopicNotPass(\''+value+'\')">'+ '<span><i class="fa fa-times"/>拒绝</span></a></shiro:hasPermission>'
                }
            }]
    });
    // 查询
    ...
    //通过
    function examinTopicPass(s) {
        window.parent.layer.confirm("确认通过?", {icon: 3, offset: 't'}, function () {
            $.ajax({
                url: '${ctx}/sys/examinteacher/examinTopicPass/' + s,
                type: 'get',
                success: function (response) {
                    if (response.code == 0) {
                        ...
                        window.location.href = '${ctx}/sys/examinteacher/examin'
                    }
                }
            });
        })
    ...
    //未通过
    function examinTopicNotPass(s) {
        window.parent.layer.confirm("确认拒绝?", {icon: 3, offset: 't'}, function () {
            $.ajax({
                url: '${ctx}/sys/examinteacher/examinTopicNotPass/' + s,
                type: 'get',
                success: function (response) {
                    if (response.code == 0) {
                        window.parent.layer.msg(response.msg, {icon: 1, time: 1000, offset: 't'});
