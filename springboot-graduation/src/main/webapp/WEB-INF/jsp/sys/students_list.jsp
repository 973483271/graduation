...
<div class="page-title">
    ...
    <div  class="search_row" style="float: right">
        <div class="form-group" style="float:left;width: 102px" >      <%-- 通过左浮动使多个div在一行显示--%>
            <div class="search_label" >学号:</div>    <input type="text" class="editText" id="stuId" onkeyup="this.value=this.value.replace(/\D/g,'')" />
        </div>
        <div class="form-group" style="float:left;width: 102px" >
            <div class="search_label" >姓名:</div>    <input type="text" class="editText" id="stuName" onkeyup="this.value=this.value.replace(/[\d]/g,'')"  />
        </div>
        <div class="form-group" style="float:left;width: 102px" >
            <div class="search_label" >班级:</div>    <input type="text" class="editText" id="stuClass" onkeyup="this.value=this.value.replace(/\D/g,'')" />
        </div>

        <div class="form-group" style="float:left" >
            <div class="search_label">学院:</div>
            <select class="" id="stuCollege" style="height: 25px" >
                <option value="">
                    全部
                </option>
        ...
        <div class="form-group" style="float:left" >
            <div class="search_label">专业:</div>
            <select class="" id="stuEducation" style="height:25px" >
                <option value="">
                    全部
                </option>
                <c:forEach var="coll" items="${eduColl}">
        ...
        <div class="form-group" style="float:left" >
            ...
            <button class="editText " type="button" id="btn-search" style="position: relative;top: 18px;left: 2px">搜索</button>
        </div>
    </div>
</div>
<div class="x_panel">
    ...
    <div class="x_content">
        <div id="toolbar" class="btn-toolbar">
            <div class="btn-group">
                <a href="${ctx}/sys/admin/addStudent" class="btn btn-default btn-sm"><i class="fa fa-plus"></i>新增</a>
...
<script>
    var list_url = '${ctx}/sys/admin/studentsList';
    // 初始化表格数据
    ...
    $('#btn-update').click(function () {
        ...
        if (rows.length == 0) {
            ...
            window.location.href = '${ctx}/sys/admin/studentsUpdate/' + rows[0].stuId;
        }
    });
    ...
    $('#btn-delete').click(function () {
        ...
        if (rows.length == 0) {
            ...
            window.parent.layer.confirm("确认删除?", {icon: 3, offset: 't'}, function () {
                $.ajax({
                    url: '${ctx}/sys/admin/studentsDelete/' + rows[0].stuId,
                    type: 'get',
            ...
            window.parent.layer.confirm("确认批量删除?", {icon: 3, offset: 't'}, function () {
                ...
                $.ajax({
                    url: '${ctx}/sys/admin/studentsDeleteBatch',
    ...
    $("#btn-batchAdd").click(function () {
        alert("请选择字段名为(用户名,姓名,班级,学院(部),专业,联系电话,邮箱)的Excel表导入");
        $("#importExel").click();
    });
    $(document).off('change','#importExel').on('change','#importExel',function(){
        ...
        $.ajax({
            url: '${ctx}/sys/admin/studentBatchAdd',
            type: 'post',
